...
export const fetchProfileAction = (username, token) => async (dispatch) => {
  ...
  try {
    ...
    dispatch({
      type: profilePageTypes.FETCH_PROFILE_SUCCESS,
      payload: profile,
    });
  } catch (err) {
    dispatch({
...
export const fetchAdditionalUsersAction =
  (userId, followingRef, stateRefLength, token) => async (dispatch) => {
    try {
      dispatch({ type: profilePageTypes.FETCH_START });
      const response = followingRef
        ? await retrieveUserFollowing(userId, stateRefLength, token)
